# app.py — Map & Gaps (polished UI + clustered markers + optional heatmap + robust guards)

import streamlit as st
st.set_page_config(page_title="Agil EV MVP • Map & Gaps", layout="wide")
...
# ---------------------------------------------------------------------
# Optional UI helpers (safe fallback if utils/ui.py does not exist)
# ---------------------------------------------------------------------
def _fallback_set_page(title, icon=None):
    st.title(title)
def _fallback_sidebar_nav(_file):
    pass
def _fallback_breadcrumbs(items):
    pass
def _fallback_status_pills():
    pass
...
set_page("🗺️ Agil EV MVP • Map & Gaps")
sidebar_nav(__file__)
breadcrumbs([("Home", "pages/0_Home.py"), ("Map & Gaps", "app.py")])
status_pills()
...
PROCESSED_CSV = DATA_DIR / "processed_sites.csv"  # optional (if you have scores/distances)
...
def load_csv(path: Path) -> pd.DataFrame:
    """Load CSV safely → always return DataFrame (or empty DF). Coerce basic types."""
    try:
        if not path.exists():
            return pd.DataFrame()
        df = pd.read_csv(path)
...
# ---------------------------------------------------------------------
# Governorates reference (centroids + pop)
# ---------------------------------------------------------------------
GOV_COORDS = {
...
with tab_rank:
    ...
    st.info("We map each Agil station and each public charger to the nearest governorate and compare "
    ...
    # Build governorate mapping (stations & chargers)
...
with tab_forecast:
    ...
    st.line_chart(df_forecast.set_index("year")[["ev_stock","chargers_needed"]])
    st.caption("**Tip:** Start conservative (e.g., EV/charger = 18–25) and stress-test in the Risk tab.")

    # Save forecast to session & allow download
    try:
...
with tab_risk:
    ...
    st.bar_chart(hist.set_index("year")["count"])
